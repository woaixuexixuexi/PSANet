...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='ourmodel')  #
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split3_vgg.yaml',
...
def get_model(args):
    ...
    optimizer = torch.optim.SGD(
                [{'params': model.parameters()}], lr=args.base_lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    if args.resume:
        ...
        if os.path.isfile(resume_path):
            ...
            prototype_neg_dict = {key: value.cuda() for key, value in checkpoint['prototype_neg_dict'].items()}
    ...
    time.sleep(5)
    return model, optimizer, prototype_neg_dict
...
def main():
    ...
    model, optimizer, prototype_neg_dict = get_model(args)
    if main_process():
        logger.info(model)
    if main_process() and args.viz:
        result_folder = os.path.join(args.result_path, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    if args.data_set == 'fss':
        train_data = dataset.DatasetFSS(datapath=args.data_root, split='trn', shot=1, transform=train_transform)
    ...
    if args.evaluate:
        ...
        if args.data_set == 'fss':
            val_data = dataset.DatasetFSS(datapath=args.data_root, split='val', shot=1, transform=val_transform)
        if args.data_set == 'pascal' or args.data_set == 'coco':
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     prototype_neg_dict = dict()
            ...
            # ----------------------  TRAIN  ----------------------
        loss_train, mIoU_train, mAcc_train, allAcc_train, prototype_neg_dict = train(train_loader, val_loader, model, optimizer, epoch, prototype_neg_dict=prototype_neg_dict)
        ...
        if args.evaluate and epoch % 1 == 0:
            ...
            if mIoU > best_miou:
                ...
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(), 'prototype_neg_dict': prototype_neg_dict}, filename)

        print('Best_step:{} \t mIoU:{:.4f} '.format(best_epoch, best_miou))
    ...
    if main_process():
        print('\nEpoch: {}/{} \t Total running data_settime: {}'.format(epoch_log, args.epochs, total_time))
...
def train(train_loader, val_loader, model, optimizer, epoch, prototype_neg_dict=None):
    global best_miou, best_FBiou, best_piou, best_epoch, keep_epoch, val_num
    ...
    pcl_pc_meter3 = AverageMeter()
    pcl_nc_meter4 = AverageMeter()
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1, aux_loss2, aux_loss3, pcl_cp, pcl_nc, prototype_neg_dict = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, epoch=epoch, cat_idx=subcls, prototype_neg_dict=prototype_neg_dict)

        loss = main_loss + args.aux_weight1*aux_loss1 + args.aux_weight2*aux_loss2 + args.aux_weight3*aux_loss3 + args.aux_weight4*pcl_cp + args.aux_weight5*pcl_nc
        ...
        aux_loss_meter3.update(aux_loss3.item(), n)
        pcl_pc_meter3.update(pcl_cp.item(), n)
        pcl_nc_meter4.update(pcl_nc.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'loss_mul {aux_loss_meter2.val:.4f} '
                        'loss_attention {aux_loss_meter3.val:.4f} '
                        'pro_pc_loss {pcl_pc_meter3.val:.4f} '
                        'pro_nc_loss {pcl_nc_meter4.val:.4f} '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          pcl_pc_meter3=pcl_pc_meter3,
                                                          pcl_nc_meter4=pcl_nc_meter4,
            ...
            if args.viz:
                ...
                writer.add_scalar('loss_mul', aux_loss_meter2.val, current_iter)
                writer.add_scalar('loss_attention', aux_loss_meter3.val, current_iter)
                writer.add_scalar('loss_pcl_pc', pcl_pc_meter3.val, current_iter)
                writer.add_scalar('loss_pcl_nc', pcl_nc_meter4.val, current_iter)
        ...
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 100 and epoch % 1 == 0 and epoch > 0) and (
            ...
            if mIoU > best_miou:
                ...
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save({'epoch': epoch - 0.5, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(), 'prototype_neg_dict': prototype_neg_dict}, filename)
    ...
    if main_process():
        logger.info(
            'Train result at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch, args.epochs, mIoU, mAcc, allAcc))
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, class_id) in enumerate(val_loader):
            ...
            # subcls = class_id.cpu().numpy()[0] - 520
