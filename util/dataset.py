...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, filter_intersection=False):    
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        if filter_intersection:  # filter images containing objects of novel categories during meta-training
            if set(label_class).issubset(set(sub_list)): #train
...
class SemData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_b_path = os.path.join(self.base_path,label_path.split('/')[-1])
        label_b = cv2.imread(label_b_path, cv2.IMREAD_GRAYSCALE)
        if label_b is None:
            print(os.path.join(self.base_path, label_path.split('/')[-1]))
        if image is None or label is None or label_b is None:
            print("Path of None",image_path)
        ...
        if self.mode == 'train':
            ...
            return image, label, label_b, s_x, s_y,  subcls_list, raw_label, raw_label_b, class_chosen
        elif self.mode == 'demo':
...
def make_GFSS_dataset(split=0, data_root=None, data_list=None, sub_list=None, sub_val_list=None, dataset='pascal'):
    assert split in [0, 1, 2, 3]
    if not os.path.isfile(data_list):
        raise (RuntimeError("Image list file do not exist: " + data_list + "\n"))
...
class GSemData(Dataset):
    ...
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        self.sub_class_list_sup, self.data_list = make_GFSS_dataset(split, data_root, data_list, self.sub_list, self.sub_val_list, self.dataset)
    ...
    def __getitem__(self, index):
        ...
        class_chosen = label_class_novel[random.randint(1,len(label_class_novel))-1]
        ...
        # Return
        if self.mode == 'val':
            return image, label_t, s_x, s_y, subcls_list, raw_label_t, class_chosen
        elif self.mode == 'demo':
            total_image_list = support_image_list_ori.copy()
            total_image_list.append(raw_image)            
